...
export interface AccountAutomationBrowserHost {
   ...
   * profileIdOverride：仅指纹宿主消费——调用方显式锚定目标窗口
   * （自动化 preflight＝活跃账号绑定 ?? 默认窗口；指纹导入＝「导入来源」选定窗口）。
   */
  readToken(profileIdOverride?: string): Promise<string>
   ...
   * expectedAccountId：被处理账号的 userId 前缀——指纹宿主在发起删除前做归属守门，
   * 页面会话属于其他账号时转协议链复核（外部宿主暂无页内 token 读取，忽略此参数）。
   */
