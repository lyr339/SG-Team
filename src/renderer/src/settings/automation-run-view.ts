...
import { automationDurationText, isActiveAutomationPhase } from './settings-view'
...
export interface AutomationStageView {
  ...
  /** 倒计时（仅 countdown / hardening-countdown 相位的当前步）。 */
...
export interface AutomationHandoverView {
  ...
  /** 用户语句：准备中 / 等待 Cursor 确认 / 已切换 / 切换失败（失败附原因，切换前等待附秒数）。 */
  detail: string
  /** 服务端原始子状态消息，供悬停排障。 */
  rawMessage: string
}
...
export const AUTOMATION_STAGE_ORDER: readonly AutomationStageKey[] = ['prepare', 'process', 'harden', 'finish']
...
const RUNNING_STAGE: Record<Exclude<AccountAutomationPhase, 'idle' | 'done' | 'failed' | 'cancelled'>, AutomationStageKey> = {
  countdown: 'prepare',
  processing: 'process',
...
function stage(key: AutomationStageKey, state: AutomationStageState, detail?: string, countdown?: AutomationStageView['countdown']): AutomationStageView {
...
/** 以某一步为界铺开四个阶段：之前完成、该步取给定状态、之后未执行 / 等待。 */
function stagesAround(at: AutomationStageKey, state: AutomationStageState, detail: string | undefined, after: AutomationStageState, countdown?: AutomationStageView['countdown']): AutomationStageView[] {
  const index = AUTOMATION_STAGE_ORDER.indexOf(at)
  return AUTOMATION_STAGE_ORDER.map((key, position) => {
    if (position < index) return stage(key, 'done')
    if (position === index) return stage(key, state, detail, countdown)
    return stage(key, after)
  })
}
...
function handoverView(run: AccountAutomationRun): AutomationHandoverView | undefined {
  ...
  if (handover.status === 'done' && handover.finishedAt) {
    ...
    // 「切换前等待」期间服务端消息形如「票据已就绪，4.5s 后切换」——把秒数提出来，其余术语不上屏。
...
export function automationRunView(input: {
  ...
  const base = {
    ...
    cancellable: run.phase === 'countdown' || run.phase === 'hardening-countdown',
    durationText: automationDurationText(run),
    handover: handoverView(run)
  }

  if (run.phase === 'done') {
    return { ...base, stages: AUTOMATION_STAGE_ORDER.map((key) => stage(key, 'done')), summary: run.message }
  }

  if (run.phase === 'failed' || run.phase === 'cancelled') {
    const trackedStage = lastActivePhase && isActiveAutomationPhase(lastActivePhase)
      ? RUNNING_STAGE[lastActivePhase as keyof typeof RUNNING_STAGE]
      : undefined
    const at = trackedStage ?? automationTerminalStageHint(run.message)
    const state: AutomationStageState = run.phase === 'failed' ? 'failed' : 'cancelled'
    return { ...base, stages: stagesAround(at, state, run.message || undefined, 'skipped') }
  }

  if (run.phase === 'idle') {
    return { ...base, stages: AUTOMATION_STAGE_ORDER.map((key) => stage(key, 'waiting')) }
  }

  const at = RUNNING_STAGE[run.phase]
  const remaining = countdownSeconds(run.remainingSec)
  if (run.phase === 'countdown' && remaining !== undefined) {
    return {
      ...base,
      stages: stagesAround(at, 'running', `${remaining} 秒后开始处理当前账号`, 'waiting', {
        remainingSec: run.remainingSec!,
        totalSec: countdownTotalSec.beforeProcess
      })
    }
  }
  if (run.phase === 'hardening-countdown' && remaining !== undefined) {
    return {
      ...base,
      stages: stagesAround(at, 'running', `${remaining} 秒后加固账号`, 'waiting', {
        remainingSec: run.remainingSec!,
        totalSec: countdownTotalSec.beforeHardening
      })
    }
  }
  return { ...base, stages: stagesAround(at, 'running', run.message || undefined, 'waiting') }
}
