 ...
 * - roles：离线入组席位的成员身份迁移（组角色 + lead 移交给池内独立席位，可附带上下文文档，
 *   阶段 2 · 2C：绑定 / 令牌 / Composer 不动）；
 * - context：上下文交接（独立席位与入组席位一致：把 Cursor 转录文档排进本会话
...
export const ROLES_HANDOFF_TITLE = '把该席位的组内身份（组角色与 lead）迁移给池内其他独立席位，可同时交接上下文文档'
...
export function resolveHandoffEntry(input: {
  ...
  // 走到这里 run 一定是 running（两态之一）：有绑定且离线的入组席位走成员身份迁移。
