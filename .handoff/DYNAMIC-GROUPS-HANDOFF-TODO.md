...
> **状态（2026-09-15 15:40）：阶段 1 代码侧完成**——切片 ①–⑥ 全部落地并全绿（详见 §13），分支 `feat/dynamic-groups-phase1` 待合并；剩余唯一一步是 §11 实机验收（用户在真机执行），通过后合回 main 并进入阶段 2。
...
>
> 基线：`2b498f9`（v0.2.1）；09-15 16:30 已 rebase 到 main `f57e4dc`（cursor-usage 修复），可 fast-forward 合回。本任务在独立 worktree `../SG-Team-groups`、分支 `feat/dynamic-groups-phase1` 上进行，
...
| 09-14 15:35–15:52 | 切片④ | `channel_outbox.kind`（user / internal / membership，旧行按标题前缀推断）；`buildMembershipNotice` 四模板 + `buildMembershipNoticeSuffix`；instructions 预告成员关系通知、`team_*` 禁令改「未入组时」；`not_in_group` 统一 `nextAction`；`team_check_in` 组简报（组目标 / lead / 无 lead 工作流）；`team_run` 池语义（start → `not_applicable`，transfer / claim / clear 写组 acting lead） | `channel-protocol-policy` +5、`channel-message-relay` +1；全量 1795 用例全绿；提交 `59ec8ca`（rebase 前 `ffa22fb`） |
| 09-15 13:00–13:25 | 切片⑤a | （CH-1 接手）`application/team-group-service.ts`：create / addMembers / remove / setLead / updateGoal / dissolve——仓储事务为真相源，成功后副作用 fail-soft：成员关系通知（`bridge.sendMessage kind:'membership'`）、出组 `releaseAgentWork`、解散 `closeGroup`、孤儿回执、lead 收「成员已移出」notice、改目标全员 notice；协作库新增 `orphanPendingReceipts`（queued / sending → `not_required`，已投递只追加 detail）+ 清扫器跳过孤儿；`project()` 池 run 去掉「团队目标」「团队已经运行」两条 blocker；IPC `team-group:*` 六个 + preload + `DesktopApi` + 主进程装配（构造在 failover 之后） | `tests/team-group-service.test.ts` 9 例、`register-team-group-ipc.test.ts` 2 例、`team-collab-sweeps` +1；全量 182 文件 / 1807 用例、typecheck、knip 全绿；提交 `5b848cd`（rebase 前 `415de98`） |
| 09-15 13:30–13:50 | 切片⑤b | 最小 UI：`run-view.ts` 独立模式改为「池内全部席位」（入组席位不再消失，席位行显示「组 · 角色」），新增 `RunGroup / RunUngroupedSeat / buildRunGroups / groupActionConsequence`；新组件 `run/RunGroupsPanel.tsx`（组卡片：名称 / 目标（可改）/ 成员状态点 / lead 标记 / attention / 已解散只读；操作：建组与加人共用抽屉——勾选未入组席位 + 每人模板下拉（默认 specialist，无 solo）+ 可空 lead；移出 / 解散各一次确认，复用 `ReplaceRunSheet`；lead 下拉直接生效；池结束后只留移出 / 解散）；`RunPage.groupActions` → `App.tsx` 接 6 个 `DesktopApi` 方法；预览 `?independent=groups` + `preview-shots` `run-independent-groups(-drawer)-{light,dark}`，深浅色截图走查通过 | `run-view.test.ts` +3、`run-groups-panel.test.tsx` 5 例；全量 183 文件 / 1815 用例、typecheck、knip 全绿；提交 `c8bac1e`（rebase 前 `6fa673f`） |
| 09-15 13:55–15:40 | 切片⑥ | （CH-4 写、CH-3 修断言、CH-1 收口）`tests/channel-mcp-stdio-groups.integration.test.ts`：真实拉起 `tsx src/mcp/index.ts` 子进程共用同一 SQLite，本进程扮演桌面端（`TeamGroupService` 直写）——solo `not_in_group` + `nextAction` → 用户投递 / 围栏退役基线 → 建组不换令牌、成员关系通知经 `check_messages` 独立后缀投递且不开回复守门 → `team_check_in` 组简报 → 两组任务隔离（`available` / `board`）→ 移出：出组通知、`not_in_group`、lead 收 notice、定向任务清定向 → 三席令牌与围栏全程不变 → 审计事件序列。测试暴露产品缺口并修复：`releaseAgentWork` 只对被移出者**持有**的 attempt 清定向，lead 规划时 `targetSlotId` 指向它但尚未被领取的任务会被 `leaseNext` / `leaseTask` / 派单器一致跳过而无人能领——现对所有定向给出组席位的非终态任务清定向（事件 `task.untargeted_by_membership`，纳入返回，lead notice 文案随之改为「与其相关的 N 项任务已回到队列」）。文档：`docs/ARCHITECTURE.md` 运行模式一节重写为「会话池 + 协作组 + 围栏」并追加 09-15 条目；`docs/TASK-MCP.md` 新增「会话池、协作组与 `not_in_group`」一节、成员关系通知的提示分层、`team_run` 池语义 | `task-pool-group-scope` +1、stdio 集成测试 1 例；全量 184 文件 / 1817 用例、typecheck、knip、`npm run build`、`smoke:channel`、`smoke:mcp` 全绿 |
| 09-15 16:20–16:30 | 合并准备 | （CH-1）分支 rebase 到 main `f57e4dc`（CH-3 的 cursor-usage 修复，与本分支无代码交集）：仓库 110 个提交零 merge commit，故用 rebase 而非 merge 保持线性；唯一冲突 `docs/ARCHITECTURE.md` 末尾两条同日条目并列（main 的「Cache Write」在前、本分支「Session pool」在后）。切片①–⑥ 五个提交 id 随之改变（本节各行已改为新 id 并括注旧 id）。主工作树仍有 55 个未提交条目、6 个文件与本分支重叠（`main/index.ts`、`preload/index.ts`、`App.tsx`、`desktop-api.ts`、`preview-main.tsx`、`preview-shots.mjs`），合回 main 须等它们先提交，届时 fast-forward 即可 | rebase 后全量 184 文件 / 1823 用例（+6 来自 main）、typecheck、knip、`npm run build`、`smoke:channel`、`smoke:mcp` 全绿；`git merge-base --is-ancestor main` 为真 |
...
- **出组清定向覆盖「未领取」任务（§7 规则 1 的补全）**：`releaseAgentWork(agentSessionId, slotId)` 除释放其持有的 attempt / 验收外，对 run 内所有 `targetSlotId === 出组席位` 的非终态任务清空定向（不改状态、不消耗 attempt、不写 failureReason），事件 `task.untargeted_by_membership`；否则 lead 用 `plan(targetSlotId)` 派给某人、对方还没领就被移出时，任务会被 `leaseNext` / `leaseTask` / `TaskDispatcher` 一致跳过、组内无人能领。只传 `agentSessionId` 不传 `slotId` 时不做定向清理（解散走 `closeGroup`，不依赖这一步）。
- **最小 UI 的取舍（§9）**：运行页独立模式显示池内全部席位（不再只显示 `solo === true`），入组席位在席位行以「组 · 角色」标注；协作组区放在 `RunIndependentPanel` 的批次概况之下，建组 / 加人共用一张抽屉，移出 / 解散复用 `ReplaceRunSheet`（后果文案说的是组不是运行），换 lead 与改目标不确认。预览场景沿用既有 `?independent=` 参数（`independent=groups`），而不是任务书写的 `?groups=1`。会话名册（`session-rail-view.ts`）未改：角色名已随组角色变化。
