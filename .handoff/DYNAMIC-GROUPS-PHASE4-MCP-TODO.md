...
> **状态（2026-09-24）：已合回 main 并随 v0.5.0 发布（`3ccfe38` 合并，`cedb9ed` 发版）。** 4D、4B、4C、4A 四片在分支 `feat/dynamic-groups-phase4`（worktree `E:\SG-phase4`）完成，全量测试、两个 smoke、度量脚本全绿（进度见第 9 节）；D5 按建议取 (a)，4E 不做。实机验收由用户在本版之后进行，清单见第 7 节——其中第 1 步（Cursor 是否响应 `list_changed`）决定 4A 是否需要退化处理。路线图见 `DYNAMIC-GROUPS-ROADMAP.md`。
>
...
| 09-24 | 审查 + 合入 | 全分支复审：`listUnreadForRecipient` 的孤儿过滤 `notification_detail NOT LIKE` 遇 NULL 会把整行排除（旧构建可能留下 NULL detail）→ 改 `COALESCE(detail, '')`；清掉最后一处提及 ping 的注释（`75914fb`）。先把 main（`71a3d1f`，v0.4.6）合进分支（`1127815`，无冲突），在合并树上重跑全量测试 / build / 两个 smoke / knip，再以合并提交合回 main（`3ccfe38`），发版 `v0.5.0`（`cedb9ed`，tag 触发 Release 工作流，mac / windows / publish 三个 job 成功，产物齐全）。分支 `feat/dynamic-groups-phase4` 已推到 origin | 合并树：214 文件 / 2204 用例绿；GitHub Actions Release `v0.5.0` 成功，`https://github.com/lyr339/SG-Team/releases/tag/v0.5.0` |
