...
> **状态（2026-09-17）：2A、2B（2B-1 + 2B-2）、2C、2D（continuity 按池模型整体退役，见 §5.1）、2E（账本随 Composer 与绑定走，统计页三层求和，见 §6.1）已落地（分支 `feat/dynamic-groups-phase2`，worktree `E:\SG-phase2`，基于 main `8e752ff`，本地未推送）；2F（可选）待动工。** 路线图见 `DYNAMIC-GROUPS-ROADMAP.md`。
