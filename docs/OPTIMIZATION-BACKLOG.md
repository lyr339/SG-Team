...
> ProcessBlocks ARIA、knip 接入、最小 CI、SessionWorkspace / App 组件测试等）见 git 历史与 `docs/ARCHITECTURE-LOG.md` 的日期条目。
>
...
| 1.3 | `legacyPatch()` 扫描为找标记读整个 20 MB workbench bundle（尾部读取即可） | P2 | S | `cursor-storage-scanner.ts`（ARCHITECTURE-LOG 09-12「Not done, recorded」） |
| 1.4 | 桌面遥测每个变化 tick 都从 `state.vscdb` `json_extract` 整段 `composerData:*` blob（09-13 已把重算从 272–545 ms 降到 13 ms；blob 读取本身仍是 O(composer 体积)） | P2 | M | ARCHITECTURE-LOG 09-12 (b) |
...
| 2.3 | 右栏审查行仍打印带零的 `+N −M`；转写文件条已改为只显示非零一侧（`describeLineCounts`）——两处应共用同一规则 | P2 | S | ARCHITECTURE-LOG 09-17「Not changed, recorded」 |
...
| # | 项 | 优先级 | 成本 | 依据 / 备注 |
|---|----|--------|------|------|
| 4.1 | ✅ 已完成（09-18）：`docs/ARCHITECTURE.md` 拆为稳定架构（108 行）+ `docs/ARCHITECTURE-LOG.md`（日期条目，append-only）；新条目只进日志。未合入的分支（阶段 3）下次 merge main 时，尾部追加的条目要手工挪进日志 | — | — | 本轮审查 |
...
| 4.3 | 启动时不校验 Cursor 版本（全部逆向锚点针对 3.6.31）；应读取 Cursor 版本并在不匹配时给出提示 | P1 | S | ARCHITECTURE-LOG 09-13「candidate follow-up」 |
...
| 5.2 | 本仓库大量 Windows 路径写的是「平台注入单测覆盖、未在 Windows 真机验证」（CDP 重启、taskkill、cmd 引号、保留端口、热切泵、NSIS 安装）；有 Windows 机器时按 ARCHITECTURE-LOG 各条目的「Still to be confirmed」清单过一遍 | P1 | M | ARCHITECTURE-LOG 09-12 / 09-13 |
