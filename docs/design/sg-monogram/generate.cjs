...
const FILES = [
  ...
  ['icon-shiguang-small.svg', SMALL, '小尺寸版（≤64px：去光晕、光刃加宽到 70，保证 16/32px 仍可读）'],
  ...
  ['trayTemplate.svg', TRAY, 'macOS 菜单栏模板图（纯黑 + alpha，系统按明暗上色）：字母剪影贴满画布，光刃变成斜向缺口']
]

for (const [name, overrides, what] of FILES) {
  const note = `<!-- 拾光 SG 字母标 · ${what}
     字形取自 Futura Bold（macOS 自带）的 S / G 轮廓并同色描边加粗 32px；光刃 28° 斜贯整块底板、两端收尖，落在 S/G 交界上。
