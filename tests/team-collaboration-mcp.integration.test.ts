...
describe('SG Team collaboration MCP', () => {
  it('moves a lead directive through inbox, read and correlated response tools', async () => {
    const path = join(mkdtempSync(join(tmpdir(), 'sg-collab-mcp-')), 'team.sqlite3')
    ...
    try {
      ...
      expect(leadTools).toEqual(expect.arrayContaining(['team_task', 'team_tasks', 'team_message']))
      // 单服务器暴露超集；主控专用动作（plan / board / broadcast / collect）对实现席按调用围栏拒绝
      expect(builderTools).toEqual(leadTools)
      const fencedPlan = await builder.client.callTool({
        name: 'team_task',
        arguments: { channel_id: '1', action: 'plan', tasks: [{ key: 'x', title: '越权规划' }] }
      })
      ...
      const inbox = await builder.client.callTool({
        name: 'team_message',
        arguments: { channel_id: '1', action: 'inbox' }
      })
      ...
      await builder.client.callTool({
        name: 'team_message',
        arguments: { channel_id: '1', action: 'read', messageId }
      })
      ...
      await builder.client.callTool({ name: 'team_task', arguments: { channel_id: '1', action: 'claim' } })
      await builder.client.callTool({ name: 'team_task', arguments: { channel_id: '1', action: 'start' } })
      const progress = await builder.client.callTool({
        name: 'team_task',
        arguments: { channel_id: '1', action: 'progress', progress: 60, summary: '接口实现完成，正在补测试。' }
      })
      ...
      await builder.client.callTool({
        name: 'team_message',
        arguments: { channel_id: '1', action: 'read', messageId: broadcastMessageId }
      })
      await builder.client.callTool({
        name: 'team_message',
        arguments: { channel_id: '1',
      ...
      const collected = await lead.client.callTool({
        name: 'team_message',
        arguments: { channel_id: '1', action: 'collect', messageIds: [broadcastMessageId] }
      })
