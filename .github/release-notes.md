...
- **应用内软件更新（macOS）**：设置 › 软件更新 在 mac 上可用——检查、下载并一键替换安装，完成后自动重开。不依赖开发者证书：下载后本地核对 sha512、版本号、Bundle ID 与签名完整性，任一不符拒绝安装、原包不动。更新源是随 Release 上传的静态清单 `update-manifest.json`；只有发布页而没有 mac 安装包的版本，面板会直接引导去发布页。
- **更新前自动备份、可回滚**：安装前把旧版应用与数据库快照存进数据目录 `updates/backup/`（只留最近一份）；更新后设置页出现「回滚到 x」，回滚连数据库一起回到更新前（更新后产生的记录另存不删），需要显式确认。替换或回滚失败会自动恢复原版本，下次启动在设置页给出一次性说明。
- **下载进度分两段**：传输完成后进入「正在核对签名与版本」的校验段，进度条走满、速率收起。
